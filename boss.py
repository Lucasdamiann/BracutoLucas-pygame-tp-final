...
class Boss:
    def __init__(self,move_x,move_y,position_x,position_y,direction,speed_walk,speed_run,power_jump,amount_gravity,frame_rate_ms,move_rate_ms,p_scale,walk_ms=100,run_ms=200,stay_ms=300,jump_ms=300,attack_ms=400,walk_on=True,run_on=True,stay_on=True,jump_on=True,attack_on=True,die_on=True):

        self.walk_r_action = Auxiliar.getSurfaceFromSeparateFiles(RESOURCES_FOLDER+BOSS_FOLDER+"WALK\_WALK_00{0}.png",0,7,flip=False,scale=p_scale)
        self.walk_l_action = Auxiliar.getSurfaceFromSeparateFiles(RESOURCES_FOLDER+BOSS_FOLDER+"WALK\_WALK_00{0}.png",0,7,flip=True,scale=p_scale)
        self.idle_r_action = Auxiliar.getSurfaceFromSeparateFiles(RESOURCES_FOLDER+BOSS_FOLDER+"IDLE\_IDLE_00{0}.png",0,7,flip=False,scale=p_scale)
        self.idle_l_action = Auxiliar.getSurfaceFromSeparateFiles(RESOURCES_FOLDER+BOSS_FOLDER+"IDLE\_IDLE_00{0}.png",0,7,flip=True,scale=p_scale)
        self.attack_r_action = Auxiliar.getSurfaceFromSeparateFiles(RESOURCES_FOLDER+BOSS_FOLDER+"ATTACK\ATTACK_00{0}.png",0,7,flip=False,scale=p_scale)
        self.attack_l_action = Auxiliar.getSurfaceFromSeparateFiles(RESOURCES_FOLDER+BOSS_FOLDER+"ATTACK\ATTACK_00{0}.png",0,7,flip=True,scale=p_scale)
        self.die_r_action = Auxiliar.getSurfaceFromSeparateFiles(RESOURCES_FOLDER+BOSS_FOLDER+"DIE\_DIE_00{0}.png",0,7,flip=False,scale=p_scale)
        self.die_l_action = Auxiliar.getSurfaceFromSeparateFiles(RESOURCES_FOLDER+BOSS_FOLDER+"DIE\_DIE_00{0}.png",0,7,flip=True,scale=p_scale)
        self.jump_r_action = Auxiliar.getSurfaceFromSeparateFiles(RESOURCES_FOLDER+BOSS_FOLDER+"JUMP\_JUMP_00{0}.png",0,7,flip=False,scale=p_scale)
        self.jump_l_action = Auxiliar.getSurfaceFromSeparateFiles(RESOURCES_FOLDER+BOSS_FOLDER+"JUMP\_JUMP_00{0}.png",0,7,flip=True,scale=p_scale)
        self.run_r_action = Auxiliar.getSurfaceFromSeparateFiles(RESOURCES_FOLDER+BOSS_FOLDER+"RUN\_RUN_00{0}.png",0,7,flip=False,scale=p_scale)
        self.run_l_action = Auxiliar.getSurfaceFromSeparateFiles(RESOURCES_FOLDER+BOSS_FOLDER+"RUN\_RUN_00{0}.png",0,7,flip=True,scale=p_scale)
        ...
        self.rect_vision = pygame.Rect(self.rect.x,self.rect.y,self.rect.w,self.rect.h)
        self.is_dead = False
        self.die_sound = pygame.mixer.Sound("Juego_freeknight\mis_assets\Sounds\mLvDeath.wav")
        self.attack_sound = pygame.mixer.Sound("Juego_freeknight\mis_assets\Sounds\mAssassinAttack2.wav")
        self.life = 2
    ...
    def stay(self):
        ...
        self.move_y = 0
        self.is_jump = False  
        if PRINTS: print("Me quedo piolita")  
    ...
    def attack(self):
        if self.direction == DIRECTION_R:
            ...
            self.move_x = 0
            if PRINTS: print("Estoy pegando a la derecha")  

        else:
            self.animation = self.attack_l_action
            self.move_x = 0
            if PRINTS: print("Estoy pegando a la izquierda")  
    ...
    def run(self):
        if self.direction == DIRECTION_R:
            ...
            if PRINTS: print("Estoy corriendo a la derecha")  
        else:
            self.animation = self.run_l_action            
            self.move_x = -self.speed_run
            if PRINTS: print("Estoy corriendo a la izquierda")  
    ...
    def automove(self,player):
        ...
        if self.is_dead == False:
            ...
            if self.direction == DIRECTION_L and self.time_acumulator <= self.walk_ms and self.walk_on:
                ...
                self.stay()
                self.time_acumulator += 1
            elif self.stay_ms < self.time_acumulator <= self.attack_ms and self.attack_on:
                self.attack()   
                self.time_acumulator += 1  
    ...
    def draw(self,screen):
        ...
        if DEBUG:   
            ...
            pygame.draw.rect(screen,C_RED,self.rect_vision,5)
